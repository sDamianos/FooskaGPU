...
void writeFieldRestart(int rank, MESH *mesh, FIELD *field){
	...
	float *h_consVec = (float*)malloc((mesh->nElem + mesh->nGhostElem)*5*sizeof(float));
	cudaMemcpy(h_consVec, field->consVec, (mesh->nElem + mesh->nGhostElem)*5*sizeof(float), cudaMemcpyDeviceToHost);
	...
	for(int iel = 0; iel < mesh->nElem; iel++){
		fprintf(f1, "%d %.10e %.10e %.10e %.10e %.10e\n",iel, h_consVec[iel*5+0], h_consVec[iel*5+1], h_consVec[iel*5+2], h_consVec[iel*5+3], h_consVec[iel*5+4]);
	}
	fclose(f1);
...
void readField(int nElem, int rank, float *d_consVec){
	
	float *h_consVec = (float*)malloc(nElem*5*sizeof(float));
	...
	for (int iel = 0; iel < nElem; iel++) {
		...
		int ret = fscanf(f, "%d %f %f %f %f %f",&idx, &v0, &v1, &v2, &v3, &v4);
		if (ret != 6) {
	...
	cudaMemcpy(d_consVec, h_consVec,nElem*5*sizeof(float), cudaMemcpyHostToDevice);
	free(h_consVec);
