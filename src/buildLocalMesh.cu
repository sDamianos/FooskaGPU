...
void localizeFaceValues(HOSTLOCALDATA *hLclData, int *lcl2glbEl, int *fc2el, int *fc2elFc, int *boundCond, float *fcCenter){
...
void localizeNodeValues(HOSTLOCALDATA *hLclData, int nGlbNodes, int max, int *glb2lclEl, int *nEl4GlbNd, int *idGlbEl4GlbNd, int *glb2lclNd, float *glbNdCord){
...
void localizeCellCenter(HOSTLOCALDATA *hLclData, int *lcl2glbEl, int *glbElNd2GlbNd, int *glb2lclNd, float *cellCenter, int *elNd2lclNd, int *nLclNodes, int *nLclNodes4glbEl){
...
void communicateVolumes(HOSTLOCALDATA *hLclData, int rank, int size, int nNeigRanks, int max, int *neigRanks, int *nProxFaces, int *neigRank4fc, int *lclProx4fc, int * lclFc2idRcv, int *fc2el, float *volume){ 
	
	int neigRank, iProx;
	float *sendBuff = (float*)malloc(size*max*sizeof(float));
	float *recvBuff = (float*)malloc(size*max*sizeof(float));
	...
	for(int iv = 0; iv < nNeigRanks; iv++){
		...
		MPI_Isend(&sendBuff[neigRank*max],nProxFaces[neigRank],MPI_FLOAT,neigRank,rank,MPI_COMM_WORLD,&reqs[r++]);
	}	
	for(int ipt = 0; ipt < nNeigRanks; ipt++){
		neigRank = neigRanks[ipt];
		MPI_Irecv(&recvBuff[neigRank*max],nProxFaces[neigRank],MPI_FLOAT,neigRank,neigRank,MPI_COMM_WORLD,&reqs[r++]);
	}
...
void buildLocalMesh(HOSTLOCALDATA *hLclData, int nGlbElem, int nGlbNodes, int max, int *nLclNodes4glbEl, int *neig, int *neigFc, int *nFaces, int *part, int *boundCond, int *nEl4GlbNd, int *idGlbEl4GlbNd, int *glbElNd2GlbNd, float *cellCenter, float *fcCenter, float *glbNdCord){
