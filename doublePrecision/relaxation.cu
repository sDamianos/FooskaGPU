...
__device__ double saturationTemp(INPUT *d_input, double P, double temp, double urf){
	
	double fun_dt, grad_f, T;

	double Pinf 	= d_input->pInf[0];
	double g1   	= d_input->gamma[0]; 
	double g2   	= d_input->gamma[1];
	double Cp1  	= d_input->Cp[0];
	double Cp2  	= d_input->Cp[1];
	double Cv1  	= Cp1/g1;
	double Cv2  	= Cp2/g2;
	double eta1 	= d_input->eta[0]; 
	double eta2     = d_input->eta[1];
	double etaTone1 = 0;
	double etaTone2 = d_input->satConst[0]*log10(P)+d_input->satConst[1];
	double As       = (Cp1 - Cp2 + etaTone2 - etaTone1)/(Cp2 - Cv2);
	double Bs       = (eta1 - eta2)/(Cp2 - Cv2);
	double Cs       = (Cp2-Cp1)/(Cp2 - Cv2);
	double Ds       = (Cp1-Cv1)/(Cp2-Cv2);
...
__global__ void partitionRelaxation(INPUT *d_input, int nElem, double urf, int *idTherm2iel, int *idChem2iel, int *nTherm, int *nChem, double *consVec){

	int iel = threadIdx.x + blockIdx.x * blockDim.x;

	int idx;
	double avf, rho, enr, tempL, pressL, Tsat;	
