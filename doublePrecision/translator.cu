...
void translator_ptw(double scale, int *nGlbElem, int *nGlbNodes, int **nFaces, int **nLclNodes, int **elNd2GlbNd, int **boundCond, double **cellCenter, double **fcCenter, double **glbNdCrdnts) {
...
// Nodes
	...
	(*glbNdCrdnts)  = (double*) malloc((ndnum*3)*sizeof(double));
	
	for(i=0;i<ndnum;i++){
		fscanf(neu,"%d\t%lf\t%lf\t%lf\n",&nd,&xc,&yc,&zc);
...
// Elements/Cells, 20012
	...
	*cellCenter = (double*) malloc((elnum*3)*sizeof(double));
	*fcCenter   = (double*) malloc((elnum*6*3)*sizeof(double));
