...
FooskaGPU is a mulit-GPU accelerated CFD software developed from scratch by me. The code is written in CUDA using cuda aware MPI for communication and thus works only in NVIDIA GPUs. You can find preliminary verisioins of both single phase and mulitphase solver here. In the mulitphase solver you can find two versions one based in double precision and one in mixed floating point precission for higher performance.
