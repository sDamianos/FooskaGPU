...
__constant__ float press1 = 11.4e5;	 __constant__ float press2 = 2.6e5;
__constant__ float temp1  = 115.3;	 __constant__ float temp2  = 115.3;
...
__global__ void initVec(INPUT *d_input, int nElem, int NEQ, float *cellCenter, double *consVec){
	
	int iel = threadIdx.x + blockIdx.x * blockDim.x;
	double avfL, avfG;
	float rhoL,rhoG,rhoMix,u,v,w,press,temp,eIntL,eIntG;
	int is_region_1 = 0;
...
void initialize(int nElem, float *cellCenter, double *consVec){
