...
__device__ __noinline__ double fntPressRlx(INPUT *d_input, double avf[2], double rho[2], double ener[2], double press[2]){
	...
	kappa = kappa*(avf[1])*m;
	double dp = (press[0] - press[1])*exp(-fabs(kappa)*d_input->dt);
	return dp;
}       
...
//		For inclusion of the finite relaxation methodology I have to compute pressure and include the dp in every residual computation, as it changes!!
		float dP = 0; float dT = 0;
/*		if(d_input->thermFntRlxFlag == 1){
			dP = fntPressRlx(d_input, avf,rho,enr,press);
			float theta;
			if(press[0]-press[1] > 1e-5){
