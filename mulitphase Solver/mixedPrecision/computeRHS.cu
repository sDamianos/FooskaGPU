...
__global__ void computeFluxProx(INPUT *d_input, int start, int nFcs, int max, int *fc2el, int *neigRank4fc, int *lclFc2idRcv, float *n, float *nt1, float *nt2, float *area, double *primVecF, float *recvBuff, double *consVec, float *RHS){
/*
	int ifc = threadIdx.x + blockIdx.x * blockDim.x;
	float solL[10];
	float solR[10];
	...
	while(ifc < nFcs){	
		...
		areaF = area[ifc];	
		for(int iv = 0; iv < 10; iv++){	
			atomicAdd( &RHS[fc2el[ifc*2+0]*10+iv], -(fluxAdv[iv] + fluxHLLC[iv])*areaF);
		}
		atomicAdd( &RHS[fc2el[ifc*2+0]*10+0], fluxNonCons[0*2+0]*areaF);
		atomicAdd( &RHS[fc2el[ifc*2+0]*10+6], -fluxNonCons[1*2+0]*areaF);
		atomicAdd( &RHS[fc2el[ifc*2+0]*10+7], -fluxNonCons[2*2+0]*areaF);
