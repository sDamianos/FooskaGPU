...
__global__ void computeViscousStressesProx(INPUT *d_input, int start, int nFcs, int max, int *fc2el, int *neigRank4fc, int *lclFc2idRcv, double *primVecF, float *grad, float *n, float *area, float *volume, float *RHS, float *recvBuffGrad, double *recvBuff);
...
__host__ void sendArrayFloat(COMM * comm, int iField, int size, float *arr);
__host__ void recvArrayFloat(COMM * comm, int iField, int size, float *arr);
