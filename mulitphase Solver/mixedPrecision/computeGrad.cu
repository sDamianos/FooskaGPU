...
__global__ void greenGaussProx(INPUT *d_input, int start, int nFcs, int max, int *fc2el, float *n, float *area, float *volume, int *neigRank4fc, int *lclFc2idRcv, double *primVecF, float *grad, float *wMin, float *wMax, float *recvBuff){
/*
	int tid = threadIdx.x + blockIdx.x * blockDim.x;
	...
	while(ifc < nFcs){
		...
		atomicMinMaxFloat(&wMin[fc2el[ifc*2+0]*10 + iv], &wMax[fc2el[ifc*2+0]*10 + iv], solR);
