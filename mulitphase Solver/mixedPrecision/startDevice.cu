...
void startDevice(int nGlbElem, int nGlbNodes, int rank, int size, HOSTLOCALDATA *hLclData, MESH *mesh, FIELD *field, COMM *comm, EXPORT *exp){
	...
	cudaMalloc((void**)&mesh->cellCenter, nElem * 3 * sizeof(float));
	cudaMalloc((void**)&mesh->fcCenter,   nFcs * 3 * sizeof(float));
	cudaMalloc((void**)&mesh->n,          nFcs * 3 * sizeof(float));
	cudaMalloc((void**)&mesh->nt1,        nFcs * 3 * sizeof(float));
	cudaMalloc((void**)&mesh->nt2,        nFcs * 3 * sizeof(float));
	cudaMalloc((void**)&mesh->area,       nFcs * sizeof(float));
	cudaMalloc((void**)&mesh->volume,     (nElem + nGhostElem) * sizeof(float));
	...
	cudaMalloc((void**)&field->RHS,           (nElem + nGhostElem) * input.NEQ * sizeof(float));
	cudaMalloc((void**)&field->grad,          (nElem + nGhostElem) * input.NEQ * 3 * sizeof(float));
	...
	cudaMalloc((void**)&field->extensionVec,  nFcs * input.NEQ * 2 * sizeof(float));
	cudaMalloc((void**)&field->theta,         (nElem + nGhostElem) * input.NEQ * sizeof(float));
	...
	cudaMalloc((void**)&comm->sendbuff,       size * nProxFacesMax * input.NEQ * sizeof(float));
	cudaMalloc((void**)&comm->recvbuff,       size * nProxFacesMax * input.NEQ * sizeof(float));
	cudaMalloc((void**)&comm->sendBuffGrad,   size * nProxFacesMax * 3 * 3 * sizeof(float));
	cudaMalloc((void**)&comm->recvBuffGrad,   size * nProxFacesMax * 3 * 3 * sizeof(float));
	...
	cudaMalloc((void**)&exp->glbNdCord,    nNodes * 3 * sizeof(float));
