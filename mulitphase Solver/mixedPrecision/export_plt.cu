...
__global__ void computeNodeVector(INPUT *d_input,int rank, int nNodes, int maxEl4nd, int nOut, int *lcl2glbNd, int *nEl4nd, int *idEl4nd, double *consVec, float *grad, float *glbNdCord, float *vecNd){

	int ilnd = threadIdx.x + blockIdx.x * blockDim.x;

	double avfL,avfG;
	float rhoL,rhoG,rho;
	float u,v,w,eIntL,eIntG;
	float press,temp,vapM;
	float gradRho,dRhoDx,dRhoDy,dRhoDz;
	int iel,ignd;
	...
	while(ilnd < nNodes){
		for(int iv = 0; iv < nEl4nd[ilnd]; iv++){
			...
			//	if(ignd==1){printf("avfMin %.10le avfL %le avfG %le subFLoat %le precise %.20le\n", d_input->avfMin, avfL, avfG, 1.0-d_input->avfMin, 1.0-(double)d_input->avfMin);}
			}
		}
...
void export_plt(EXPORT *exp, FIELD *field){
	...
	h_vecNd = (float*) malloc(nGlbNodes*nOut*sizeof(float));
	...
	if(rank == 0){
		...
		for(int ind = 0; ind < nGlbNodes; ind++){
			for(int i = 0; i < nOut; i++){
				fprintf(f1, "%e ", h_vecNd[ind*nOut + i]);
			}
			 fprintf(f1, "\n");
		}
