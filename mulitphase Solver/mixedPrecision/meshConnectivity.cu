...
__global__ void solveConnectivity(int rank, int nGlbElem, int nElemLcl, int maxEl4glbNd, int * nFaces, int * boundCond, int * elNd2GlbNd, int * nLclNodes, int * nEl4GlbNd, int * idGlbEl4GlbNd, double * fcCenter, int * neig, int * neigFc){
				...
				for(int iv = 0; iv < nEl4GlbNd[ignd]; iv++){
					...
					if(iel1 != iel){
						for(int ifc1 = 0; ifc1 < nFaces[iel1]; ifc1++){
							if( fabs(fcCenter[iel*6*3 + ifc*3 + 0] - fcCenter[iel1*6*3 + ifc1*3 + 0]) < 1e-9 && fabs(fcCenter[iel*6*3 + ifc*3 + 1] - fcCenter[iel1*6*3 + ifc1*3 + 1]) < 1e-9 && fabs(fcCenter[iel*6*3 + ifc*3 + 2] - fcCenter[iel1*6*3 + ifc1*3 + 2]) < 1e-9){
								neig[iel*6+ifc] = iel1;
...
void copyToDevice(int nGlbElem,	      int   nGlbNodes,
		  ...
		  double *h_fcCenter,  double **d_fcCenter,
	...
	cudaMalloc((void**)d_fcCenter,    nGlbElem * 6 * 3 * sizeof(double));
	...
	cudaMemcpy(*d_fcCenter,    h_fcCenter,    nGlbElem * 6 * 3 * sizeof(double), cudaMemcpyHostToDevice);
...
void meshConnectivity(int nGlbElem,int nGlbNodes,int *h_nFaces,int *h_nLclNodes,int *h_elNd2GlbNd, int *h_neig, int *h_neigFc, int *h_boundCond, int *h_nEl4GlbNd, int **h_idGlbEl4GlbNd, int *max, double *h_fcCenter){
...
//	cudaEventRecord(start,0);	
	...
	if (err != cudaSuccess) {
		printf("CUDA error in solveConnectivity: %s\n", cudaGetErrorString(err));
	}
