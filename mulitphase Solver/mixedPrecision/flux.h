__device__ __forceinline__ void fluxSource(INPUT *d_input, float *uNonCons, double *consVecL, double *consVecR, float (&n)[3], float (&fluxNonCons)[6]){

	float rhoLiq,rhoGas,eLiq,eGas,pLiq,pGas;
	double avf;
//	float avfF
...
__device__ __forceinline__ void flux(INPUT *d_input, int isStarRegion, float (&primVec)[10], float (&fluxAdv)[15], float (&n)[3], float (&uNonCons)[3], float areL, float areG){
