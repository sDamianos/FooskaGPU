...
void readMesh(HOSTLOCALDATA *hLclData, int rank){
	...
	for (int i = 0; i < hLclData->nFcs; i++) {
		...
		double fcx, fcy, fcz;
		double nx, ny, nz;
		double nt1x, nt1y, nt1z;
		double nt2x, nt2y, nt2z;
		double area;
...
void readField(int nElem, int rank, double *d_consVec){
	...
	double *h_consVec = (double*)malloc(nElem*5*sizeof(double));
	...
	cudaMemcpy(d_consVec, h_consVec,nElem*5*sizeof(double), cudaMemcpyHostToDevice);
	free(h_consVec);
