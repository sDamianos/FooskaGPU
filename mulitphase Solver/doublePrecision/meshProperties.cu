...
void normalVector(double *x, double *y, double *z, double *n){
...
void tangentVector(double *x, double *y, double *z, double *n, double *nt1, double *nt2){
...
void computeArea(int nFcNds, double *x, double *y, double *z, double *area){			
...
void computeVolume(int nElem, int iel, const int (*vnHexa)[3], const int (*vnTetra)[3], const int (*vnPrism)[3], const int (*vnPyramid)[3], int *nLclNodes, int *elNd2lclNd, double *glbNdCord, double *volume){
...
void meshProperties(HOSTLOCALDATA *hLclData, int *nLclNodes, int *fc2el, int *fc2elFc, int *elNd2lclNd){
	...
	hLclData->n    = (double*)malloc(hLclData->nFcs*3*sizeof(double));
	hLclData->nt1  = (double*)malloc(hLclData->nFcs*3*sizeof(double));
	hLclData->nt2  = (double*)malloc(hLclData->nFcs*3*sizeof(double));
	hLclData->area = (double*)malloc(hLclData->nFcs*  sizeof(double));
	...
	hLclData->volume = (double*)malloc((hLclData->nElem+hLclData->nGhostElem)*sizeof(double));
...
//		if(rank==2&&iel==5){printf("iel %d volume %f\n", iel, hLclData->volume[iel]);}
//		if(rank==2&&iel==11){printf("iel %d volume %f\n", iel, hLclData->volume[iel]);}
	}
