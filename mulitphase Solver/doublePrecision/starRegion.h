...
__device__ __forceinline__ void starRegion(INPUT *d_input,double (&sol)[10], double (&uLcl)[3], double SK,double S_S, double (&n)[3], double (&nt1)[3], double (&nt2)[3], double (&uNonCons)[3], double (&vecS)[10], double *consF)
{                   
	double u[3];
	double rhoSL,pSL,rhoSG,pSG;
	double press,rho,rhoS;
